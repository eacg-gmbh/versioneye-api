...
describe V2::OrganisationsApiV2, :type => :request do
  ...
  describe "POST /v2/organisations/test_orga/inventory_diff" do
    ...
    it "returns status code 200" do
      ...
      inv_uri = "/api/v2/organisations/#{@orga.name}/inventory_diff?api_key=#{@orga_api.api_key}"
      response = post inv_uri, params: {
        f1_team_name: "ALL",
        f1_language: "ALL",
        f1_project_version: "ALL",
        f1_post_filter: "ALL",
        f2_team_name: "ALL",
        f2_language: "ALL",
        f2_project_version: "ALL",
        f2_post_filter: "ALL"
      }

      resp_obj = JSON.parse response.body
      diff_id = resp_obj["diff_id"].to_s
      expect( response.status ).to eq(201)
      expect( diff_id ).to_not be_nil
      expect( InventoryDiff.find diff_id ).to_not be_nil
    end
  end
  ...
  describe "GET /v2/organisations/test_orga/inventory" do
    it "returns status code 200" do
      ...
      diff.save
      get "/api/v2/organisations/#{@orga.name}/inventory_diff?api_key=#{@orga_api.api_key}&inventory_diff_id=#{diff.ids}"
      expect( response.status ).to eq(200)
      response_data = JSON.parse(response.body)
      expect( response_data["organisation_id"] ).to eq(@orga.ids)
    end
  end
